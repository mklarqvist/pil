...
public:
    ...
    int Serialize(std::shared_ptr<ColumnSet> cset) {
        ...
        for(int i = 0; i < cset->size(); ++i) {
            ...
            std::cerr << "metas=" << i << "/" << cset_meta.back()->column_meta_data.size() << std::endl;
            ...
            std::cerr << "before pushing back modification" << std::endl;
            cset_meta.back()->column_meta_data[i]->last_modified = static_cast<uint64_t>(std::time(0));
            std::cerr << "after meta. all done for: " << i << "/" << cset->size() << std::endl;
        }
        writer->flush();
        return(1);
    }
...
public:
    ...
    int Serialize(std::ofstream& stream) {
        ...
        std::cerr << "done after schemas" << std::endl;
